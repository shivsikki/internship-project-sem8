...
router.post('/create', verifyToken, async (req, res) => {
  try {
    const { patientId, appointmentId, testType, testName, bodyCheck, testResults, notes, status } = req.body;
...
router.put('/:id', verifyToken, async (req, res) => {
  try {
    const { testResults, notes, status, bodyCheck } = req.body;
...
router.get('/all', verifyToken, async (req, res) => {
  try {
    ...
    if (!user || user.role !== 'admin') {
      return res.status(403).json({ success: false, message: 'Only admin can view all tests' });
    }
