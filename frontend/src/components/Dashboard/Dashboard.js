...
import DoctorVerification from '../Admin/DoctorVerification';
...
const Dashboard = ({ isSidebarCollapsed, setIsSidebarCollapsed }) => {
  ...
  if (user.role === 'patient') {
    ...
    if (user.isRootAdmin) {
      navItems.push({ id: 'admin-requests', label: 'Sub-Admin Requests' });
    }
    navItems.push({ id: 'admin-users', label: 'Manage Users' });
    navItems.push({ id: 'doctor-verification', label: 'Doctor Verification' });
    navItems.push({ id: 'appointments', label: 'Appointments' });
    navItems.push({ id: 'prescriptions', label: 'Prescriptions' });
    navItems.push({ id: 'tests', label: 'Tests' });
    navItems.push({ id: 'payments', label: 'All Payments' });
    navItems.push({ id: 'admin-analytics', label: 'Analytics' });
    navItems.push({ id: 'admin-broadcast', label: 'Broadcast' });
    navItems.push({ id: 'admin-audit', label: 'System Logs' });
    navItems.push({ id: 'admin-settings', label: 'Settings' });
  }
      ...
      case 'doctor-verification':
        return (
          <svg {...common}>
            ...
            <path d="M12 3v6l-6 10a4 4 0 0 0 3.5 6h9A4 4 0 0 0 20 18L14 8V2" />
            <line x1="8" y1="6" x2="16" y2="6" />
          </svg>
        );
      default:
        ...
        <div className="dashboard-content">
          {announcements.length > 0 && (
            <div className="announcement-banner">
              ...
              <button className="announcement-close" onClick={() => setAnnouncements(announcements.slice(1))}>×</button>
            </div>
          )}
          {activeTab === 'dashboard' && (
            <>
          ...
          {activeTab === 'admin-requests' && user.role === 'admin' && user.isRootAdmin && (
            <AdminRequests />
          )}
          ...
          {activeTab === 'admin-broadcast' && user.role === 'admin' && (
            <AdminBroadcast />
          )}
          ...
          {activeTab === 'doctor-verification' && user.role === 'admin' && (
            <DoctorVerification />
          )}
        </div>
      </main>
